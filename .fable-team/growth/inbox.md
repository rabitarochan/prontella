...
- [ ] 2026-07-27 【失敗→パターン】ファイルツリーの新規作成用インライン入力が 1〜2 秒ごとに消える不具合。原因は `FileTree.tsx` の `Node` を**コンポーネント関数の内側で定義**していたこと。react-arborist は `props.children` を「要素タイプ」として描画するため、親の再レンダーごとに identity が変わり subtree が unmount→remount される。ポーリング(App 4s / useTerminalSessions 3s)は引き金にすぎない。**行レンダラーをコンポーネント内で定義しない**を規約化したい(pj-* 候補)
- [ ] 2026-07-27 【摩擦】uncontrolled な入力(`value` なし + `autoFocus`)は remount されても「フォーカスは残り値だけ消える」ため、原因が入力処理側にあるように見えて誤誘導される。入力欄の不具合は「値が消えたか」より **DOM ノードが差し替わったか**(要素参照の同一性)を先に測ると一発で切り分けられる
- [ ] 2026-07-27 【成功パターン】ポーリング起因が疑われる UI 不具合は、隔離環境で「①値のクリア時刻列 ②要素参照の同一性反転 ③API リクエスト送出時刻」の 3 系列を同時採取すると、因果が 3 方向から裏付けられて仮説の反証余地が消える
- [ ] 2026-07-27 【失敗→パターン】CDP 検証で「blur / フォーカス遷移」を検証するとき、`element.dispatchEvent(new MouseEvent(...))`(untrusted)ではブラウザーのデフォルトのフォーカス移動が起きず、**正常な実装を誤って NG と判定しかけた**。フォーカス・blur・クリック起因の検証は必ず `Input.dispatchMouseEvent`(trusted event)を使う — pj-isolated-verify 追記候補
- [ ] 2026-07-27 【驚き】自作 CDP クライアントで `Runtime.evaluate` に `replMode: true` を付けると `awaitPromise` が効かなくなり即座に空値が返る。非同期評価を使うヘルパーでは `replMode` を付けない — pj-isolated-verify 追記候補
- [ ] 2026-07-27 【成功パターン】「ポーリングを止めただけの見かけの修正」を排除するため、修正後の検証項目に **API リクエストが従来周期のまま送出され続けていること**を明示的に入れた。症状消失の確認だけでは、原因を潰したのか引き金を潰したのか区別できない

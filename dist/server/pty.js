...
const CLAUDE_UI_RE = /claude\s*code|welcome\s*to\s*claude|anthropic|model:[^\n|]{1,40}\|\s*ctx:|shift\+?\s*tab\s*to\s*cycle|⏵⏵/i;
...
export class PtyManager {
    ...
    statusFor(cwd) {
        ...
        if (sessions.length === 0)
            return { status: 'none', terminalId: null };
        const order = ['waiting', 'busy', 'idle', 'shell'];
        for (const status of order) {
            const hit = sessions.find((s) => s.status === status);
            if (hit)
                return { status, terminalId: hit.id };
        }
    ...
    onData(session, data) {
        ...
        else if (session.claudeDetected &&
            session.status !== 'busy' &&

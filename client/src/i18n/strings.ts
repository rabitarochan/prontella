...
const CORE_STRINGS = {
  ...
  'metrics.paletteEnable': { ja: '診断: 匿名メトリクスの収集を有効にする (ローカル保存のみ)', en: 'Diagnostics: enable anonymous metrics (stored locally only)' },
  'metrics.paletteDisable': { ja: '診断: 匿名メトリクスの収集を無効にする', en: 'Diagnostics: disable anonymous metrics' },
  'metrics.paletteLocked': { ja: '診断: メトリクスは PRONTELLA_METRICS で固定されています ({tier})', en: 'Diagnostics: metrics tier is locked by PRONTELLA_METRICS ({tier})' },
  'metrics.paletteExport': { ja: '診断: 匿名メトリクスのバンドルをダウンロード', en: 'Diagnostics: download anonymous metrics bundle' },
  'metrics.railTooltip': { ja: 'メトリクス収集中: {tier}', en: 'Collecting metrics: {tier}' },

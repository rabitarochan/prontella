...
- [ ] 2026-07-28 【驚き】react-arborist の**開閉状態は `data` prop から独立した内部ストア**(`state.nodes.open.unfiltered`、ノード id がキー)に保持され、`data` を丸ごと差し替えても消えない。そのためツリーの再読み込みで `children` を `null` に戻すと「シェブロンもアイコンも開いた見た目のまま中身が 0 件」になり、ユーザーは閉じる→開くの二度手間を強いられる。一般化: **サードパーティの表示コンポーネントが持つ内部 UI 状態(開閉・選択・スクロール)は data の作り直しでは消えない**ので、再取得系を実装するときは「データと UI 内部状態の両方を整合させる」ことを設計項目に立てる — pj-client-ui-state 追記候補
- [ ] 2026-07-28 【摩擦】CDP でのクリック検証で `Input.dispatchMouseEvent` の **`mouseMoved` を挟むと 1 発ごとに約 5 秒の遅延**が乗ることがある(実測: 4 連続とも 5011〜5014ms。Chrome/接続固有で再現性あり)。連打・二重発火・競合系の検証で「連打したつもりが 17 秒間隔」になり、**製品側の stale 対策が効いているのか単に間隔が空いていただけなのか区別できなくなる**。`mousePressed`/`mouseReleased` のみでも通常クリックは正常に発火するので、タイミング依存の検証では `mouseMoved` を外す(フォーカス/hover 依存の検証では必要なので使い分ける) — pj-isolated-verify 追記候補
- [ ] 2026-07-29 【摩擦】pj-isolated-verify の「スクラッチは短パスに置く」(MAX_PATH 対策、罠 5) に従って `C:\vt-ft` にフィクスチャを作ると、**後片付けの `Remove-Item` がハーネスのトップレベルパス保護に弾かれて削除できない**(`Remove-Item on system path 'C:\vt-ft' is blocked`)。2 つの規約が衝突している。回避は迂回ではなく **1 階層深いディレクトリー(例 `C:\vt\ft-<日付>`)を使う**ことで、短パスも後片付けも両立する — pj-isolated-verify の罠 5 に追記候補

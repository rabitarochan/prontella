...
/**
 * ホスト端末を noVNC で操作するメインビュー。リポジトリー未選択の VNC モードで
 * main 全体に表示される。App 直下 (WorktreeView の key remount の外) に置くことで、
 * リポジトリー選択で VNC モードを抜けても RFB 接続が生存する
 * (visible=false の間は display:none で保持され、unmount されない)。
 *
 * 接続はユーザー操作起点のみ (自動接続しない)。VNC サーバー未起動が常態のユーザーで
 * 無駄打ちを避け、StrictMode の dev 二重マウントによる connect/disconnect の
 * 二重発火も構造的に回避する。
 */
export default function VncView({ visible }: { visible: boolean }) {
  const t = useT();
  const setActive = useVncView((s) => s.setActive);
  ...
  // ビューを表示したとき、未接続なら到達性を取り直して idle ⇄ unreachable を出し分ける
  useEffect(() => {
    if (!visible) return;
    ...
    // phase を依存に入れると接続失敗のたびに再プローブが走るため visible のみで発火する
  ...
  return (
    <section className="vnc-view" style={{ display: visible ? undefined : 'none' }}>
      <header className="vnc-view-header">
        <span className="codicon codicon-vm" />
        <span className="vnc-view-title">{t('vnc.title')}</span>
        <span className="vnc-view-actions">
          ...
          <button className="icon-btn" title={t('common.close')} onClick={() => setActive(false)}>
      ...
      <div className="vnc-screen-wrap">
        ...
        {phase !== 'connected' && (
          <div className="vnc-overlay">
            {phase === 'idle' && (
              <>
                <p className="vnc-overlay-text">
                  {status ? t('vnc.idleHint', hostLabel) : t('vnc.checking')}
                </p>
            ...
            {phase === 'connecting' && <p className="vnc-overlay-text">{t('vnc.connecting')}</p>}
            {phase === 'credentials' && (
              <form

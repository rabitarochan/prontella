...
- [ ] 2026-07-29 【摩擦】pj-isolated-verify の「スクラッチは短パスに置く」(MAX_PATH 対策、罠 5) に従って `C:\vt-ft` にフィクスチャを作ると、**後片付けの `Remove-Item` がハーネスのトップレベルパス保護に弾かれて削除できない**(`Remove-Item on system path 'C:\vt-ft' is blocked`)。2 つの規約が衝突している。→ **2026-07-29 ユーザー判断で解決**: 作業領域をリポジトリー直下の `vt/`(verify test、`.gitignore` 済み)に集約する規約に変更し、pj-isolated-verify の手順 1・罠 5/6/7・後始末に反映。ドライブ直下は今後使わない
- [ ] 2026-07-29 【驚き】inbox に記録済みの制御バイト走査式 `c<9||(c>13&&c<32)||c===127` は **0x0b(垂直タブ)と 0x0c(改ページ)を素通しする**(9-13 を丸ごと許容しているため)。実際、この式で「制御バイトなし」と判定した直後にフックが 0x0b を検出した。混入元は `C:\vt5` の `\v` がどこかの層でエスケープ解釈された残骸で、**コミット済みのまま複数の grow サイクルを生き延びていた**。正しい条件は「0x20 未満のうち 0x09(タブ)と 0x0a(LF)と 0x0d(CR) 以外、および 0x7f」。**記録済みの検出式そのものが穴を持つ**という型なので、走査式は「許可リスト方式(通してよいバイトを列挙)」で書く
- [ ] 2026-07-29 【失敗→パターン】PowerShell でファイルをバイト単位修正する際、`$list.AddRange($bytes[0..$n])` は配列スライスが `object[]` になるため **MethodException で失敗するが、スクリプトは停止せず後続の `WriteAllBytes` が実行される**。結果 37KB のファイルが 2 バイトに切り詰められた(git 管理下だったため `git checkout HEAD -- <file>` で復旧、未コミットの 1 行のみ喪失)。**バイト単位の書き換えは (1) 一時ファイルに書く (2) サイズと再走査で検証する (3) 検証が通ってから原本に上書き、の 3 段階で行う**。原本を直接 open して書くルートを取らない

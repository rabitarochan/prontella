...
- **例外: 実ユーザー環境そのものが検証対象のときは USERPROFILE を差し替えない**。
  ...
  再構成できているか」の検証。2026-08-28 実測)。**子プロセスがユーザープロフィール下の
  キャッシュや設定を読む場合も同じ**(例: 言語サーバー Roslyn は MSBuild/NuGet 経由で
  `~/.nuget/packages` を読む。隔離 home ではプロジェクトは読めても参照が解決されず、補完/ホバーが
  null になり**製品の不具合と区別がつかない**。2026-09-12 実測)。`.nuget` / `.npm` / `.cargo` /
  `.m2` のような**ホーム直下のツールキャッシュに依存する子プロセス**を起動する機能は、隔離 home では
  検証できないと最初から判断する。この場合は**ポート分離だけを維持**し、
...
20. **共有資材を「書き換える」機能は、生成部分だけ隔離ホームの別プロセスに追い出す**:
    ...
    **書き換える**コードは、ポート分離だけでは守れない — 検証用サーバーが実ファイルを
    ...
    実体 node.exe で直叩き。罠 2)、**実資材はバックアップとの `diff -q` で無傷を証明して
    報告する**。タイムスタンプだけの確認では中身の同一性を示せない
21. **API で作ったセッションはタイルに所有されないので UI に出ない**: `POST /api/terminals`
    で作った端末はサーバーには存在するが、タイルレイアウトの `leaf.sessions` に入らないため
    タブが生えず、そのセッションを対象にした UI の表示は**永久に現れない**。
    「サーバーは正しいのに UI が空」で時間を溶かす。**UI 経路を検証するなら UI の
    ボタンから作る**(作成前後で `/api/terminals` を差分して新しい id を拾う)
22. **フルスイートのフレークは「自分の追加分を外して」切り分ける**: 実ブラウザーや
    バックグラウンドサーバーを動かした後の高負荷下では、実プロセスを spawn する既存テスト
    (`git apply` 等の 5 秒タイムアウト)がフルスイートでだけ落ちる。単独実行で通ることは
    無罪証明にならない(負荷が違う)。**`--exclude` で自分が足したテストファイルを外して
    同じ負荷で再現するか**を見る。再現すれば自分の変更は無関係と言い切れる
23. **「リサイズが効いた」をピクセル幅で測らない — 子プロセス自身に言わせる**:
    ペインの `getBoundingClientRect().width` が変わっても、それは**クライアントのレイアウトが
    変わっただけ**で、PTY の winsize が追従した証明にはならない(`fit()` の失敗も
    ResizeObserver の非発火も無症状)。**シェルに自分の幅を出力させて xterm のバッファから読む**
    (PowerShell なら `"COLS=$($Host.UI.RawUI.WindowSize.Width)"`)。分割の前後で同じターミナルに
    2 回出させると、**同じバッファに 2 つの値が並ぶので「再マウントしていない」ことまで同時に
    証明できる**(2026-08-30 実測: 1302px で `COLS=183` → 分割後 645px で `COLS=90`、
    前の行も残存)。読み出しは下の「xterm の画面内容」の非表示ルートで足りる
24. **実 HOME で deck を起動したら、何を検証していようと `hook-settings.json` を戻す**:
    ...
    **撤収時に `ensureHookAssets(<本番ポート>)` を別プロセスで呼んで再生成し、
    ファイル内のポートを grep して確認する**。タイムスタンプの確認では気づけない —
    「更新されている」ことは分かっても「誰のポートで」が分からないため
25. **対話でしか決まらない既定値は非対話実行では測れない**: PSReadLine の
    `PredictionSource` は `powershell -File` で測ると 2.4.5 でも `None` を返すが、
    実 PTY の対話セッションでは `HistoryAndPlugin` になる(2026-09-01 実測)。
    **「機能が有効か」の判定を非対話実行の戻り値で行うと、正常な実装を不合格にする**。
    対話状態が要るなら **deck を立てて `POST /api/terminals` → `/ws/term?id=` から測る**。
    node-pty をスクリプトから直接呼ぶ手は使えない — コンソールを持たないプロセス
    (Bash/PowerShell ツール経由の実行) では `AttachConsole failed` で即死する
26. **ConPTY へ長い 1 行を流し込まない**: 数百文字の 1 行コマンドを `/ws/term` へ書くと、
    折り返しと再描画で崩れて実行されない(実測: 送ったのと違うコマンドがエコーされ、
    出力ファイルが永久にできず「タイムアウト」になった)。**検証コマンドは `.ps1` に置いて
    `& '<path>' -Out '<path>'` の短い 1 行で呼ぶ**。副次効果として引用符のエスケープ事故も消える
27. **「変更前の状態」を再現するときは値だけでなく順序も合わせる**: 失われた PATH 要素を
    復元して旧挙動を再現する検証では、**末尾に足すか先頭に足すかで結果が反転する**。
    2026-09-01 実測: `PSModulePath` に PS7 のパスを末尾へ足したら先勝ちで
...
- **実エージェントを再実行せずに UI を検証する — 実記録ペイロードの再生**: 外部プロセス
  (Claude Code 等)のイベントを受けて描画が変わる画面は、**一度だけ実物を走らせて
  生ペイロードを JSONL に採り、以後はそれをサーバーのエンドポイントへ順に POST する**。
  実行が決定的になり、課金も待ち時間も消え、**同じ入力で修正前後を比較できる**。
  1 発ごとに**サーバーの API とページの DOM を両方読む**と、どちらの層の遅れかが
  一目で分かる(`pj-client-ui-state` §13 の切り分け)。敵対的入力の試験も
  同じ再生器に混ぜられる
...
- **xterm の画面内容は「表示中は」DOM から読めない**。WebGL レンダラーはキャンバスへ描くため
  `.xterm-rows` が空になる。ただし **WebGL addon は `visible=false` で dispose されて DOM
  レンダラーに戻る**(`XTermView` の可視性 effect)ので、**別タブへ切り替えて非表示にしてから
  `.xterm-rows` の `textContent` を読めばよい**(2026-08-30 実測。`\u00a0` を空白へ正規化する)。
  これが最も安い経路 — スクロールバックの残存確認や、子プロセスに出力させた値の回収に使える。
  **複数のターミナルを同時に読みたいとき / 切り替え自体が観測対象を変えるとき**は、
  **`/ws/term?id=` へ検証側から別ソケットを張り、サーバーが返す snapshot を読む**。
  再アタッチ時の DEC モード再生 (bracketed paste 等) もこの snapshot の先頭プレフィックスで
  検証できる

...
## 2026-07-28 — grow 第 5 回(Markdown プレビュー実装からの 18 シグナル)

**対象**: `/fable-team:task Markdown プレビュー表示機能` で蓄積した未処理 18 件
(失敗→パターン 6 / 成功パターン 7 / 驚き 3 / 摩擦 2)。ユーザー承認済み。

### 1. [新規 pj-Skill] `pj-untrusted-input`

信頼できない入力(.md の中身・URL・パス・リクエストボディ)を分類/検証するコードの定石。
**証拠**: シグナル 92, 93, 97, 98, 99, 101, 107, 108。

決め手は **「正規化してから判定」の順序欠陥が同一関数 `classifyHref` で 3 回連続で再発した**こと
(パーセントデコードがスキーム判定より後 → `\` を区切りとして正規化していない → バックスラッシュ
正規化がプロトコル相対 `//` 判定より後)。三度目は明確にパターンで、**1 件ずつ直すと別の入り口で
再発する**ため「順序の構造ごと作り直させる」を規約化した。個別修正の繰り返しで修正サイクルを
3 回消費した実測がある。

併せて収録: 二重防御が競合したとき片方を消さない / ライブラリの防御挙動は実際の出力で確認する /
advisory 一覧を「使ってはいけない機能」リストとして読む / `npm audit --json` の `nodes` で自分の
依存か切り分ける / 制御文字を扱う実装は文字コード比較で書かせる / Conductor が自分の入力セットで
叩き直す。

**効果確認**: 「正規化と判定の順序」型のシグナルが再出現しないこと。および新しい分類関数の
実装タスクで、収録した確認セット(生 / パーセント / 多重 / `\` / UNC / 空 / `//`)が
brief に載っていること。

### 2. [pj-isolated-verify 追記] 罠 8・9 + 「フィクスチャの作り方」節

**証拠**: シグナル 95(Git Bash が curl の非 ASCII 引数を壊す。危うく虚偽のバグ報告を出すところ
だった)、100(スクラッチ配下の vitest 設定は third-party の解決経路を変える)、102(敵対的
フィクスチャは実装前に作る + 報告された欠陥は横展開する)、106(フィクスチャに合格基準そのものを
書き込む)。

106 は実際に効いた: `gfm.md` に「エディタータブを一度も開かずに開いた状態でも着色されること
(モノクロなら不合格)」と書いておいたため、**brief 本文が名指ししていなかった json の着色漏れも
verifier が自力で不合格判定できた**。

**効果確認**: 検証タスクで「shell 経由の非 ASCII で誤った不具合報告」「スクラッチ検証の所見を
本番未再現のまま採用」が起きないこと。

### 3. [pj-client-ui-state 追記] §0 テスト基盤の限界 + §5 Monaco のコールドスタート

**証拠**: シグナル 104(`vitest.config.ts` の include が `*.test.ts` のみで `.tsx` が対象外、
`@testing-library/react` も無い ⇒ **React コンポーネントは構造的に回帰テストで守れない**)、
94(`monaco.editor.colorize()` の `.mtkN` 配色 CSS はエディターインスタンス生成時にしか注入されず、
`json` のような遅延登録言語は `onLanguage` が発火しないため無配色になる)。

104 を §0(前提)に置いたのは、**「テストが通ったので安全」という報告がコンポーネントの話なら
成立していない**ことを最初に読ませるため。

**効果確認**: コンポーネント変更の完了報告で「テスト green」を無罪証明として受け取っていないこと。
Monaco 系画面の検証項目に「エディタータブを一度も開かずに」が入っていること。

### 4. [フック新設] `.claude/settings.json` PostToolUse(Write|Edit)

**証拠**: シグナル 98。`.claude/hooks/check-control-bytes.mjs` が、書き込まれたテキストソースに
不正な C0 制御バイト(0x09/0x0A/0x0D 以外の 0x20 未満と 0x7F)が入っていないか走査する。

「同じ指摘を二度させない」ための機械化。**このフック自身を書いた最初の試行にも制御バイトが
混入し、フックがそれを検出した**(セッション内 3 例目)。人手の規律だけでは足りない証拠として記録する。

**効果確認**: 制御バイト混入のシグナルが再出現しないこと。誤検知(正当なファイルへの警告)が
出るようなら拡張子リストを絞る。

### 5. [フレームワークへの提案 — 未適用・上流へ持ち出す]

プラグインキャッシュは編集していない。Fable Team リポジトリーの brief playbook 宛の提案:

- **同名概念が複数ある領域は brief で「どちらを指すか」を名指しで固定する**(証拠: シグナル 91。
  この codebase の "タブ" がタイルのペイン切替とファイルタブの 2 種あり、scout が事実は正しく
  列挙しながら推奨先を誤った概念に接続した)
- **builder 報告の「積み残し・懸念」欄は最も情報密度が高い**。主張の正否だけでなく「原因はもっと
  下にないか」まで辿る(証拠: シグナル 96。懸念欄の 1 行から下位関数の実バグが出た)
- **reviewer には推奨修正そのものの副作用まで書かせる**(証拠: シグナル 103。「素直に直すと
  第 2 のバグが出る」まで指摘してきた)
- **「未実測だが原理上こうなるはず」は仮説であって所見ではない**。仕様として記録しない
  (証拠: シグナル 105。実際に誤りで、verifier が同じ操作を 1 回やり直すだけで反証した)

### ルールダイエット

- `pj-git-route` §2 冒頭の Phase 4 / Phase 5 の経緯 4 行を 1 行に圧縮(-3 行)。**読み手の行動を
  変えない履歴**で、残すべき判断(先頭 `-` 拒否が主防壁)だけを残した。第 4 回で
  `pj-isolated-verify` に施したのと同じ処置。空いた行で `pj-untrusted-input` への相互リンクを追加
- **前回の宿題への回答**: `pj-isolated-verify` 罠 4(選択中 worktree の削除失敗)は今回も
  再現確認できていない(今回の作業と無関係で、確認には実 worktree の作成・削除が要る)。
  削除せず **「2026-07-21 以降未再現確認」と明記**した。次に worktree 周りを触るときに確かめ、
  直っていれば削除する
- pj-* は 4 本になった。次回は 4 本横断で重複と死文を点検する

---

## 2026-07-29 — /fable-team:grow(第 2 回。対象シグナル 12 件)

対象ミッション: Git タブのブランチ upstream 連携 3 機能(バッジ / 早送りフェッチ / 別名プッシュ)。

### 0. [有効性確認 — 前回サイクルの成果] pj-isolated-verify「フィクスチャの作り方」は効いた

- **確認できたこと**: 前回 harvest した 2 ルール —「敵対的フィクスチャは実装前に Conductor が作る」
  「フィクスチャの中に合格基準そのものを書き込む」— が今回そのまま発火した。実装前に作った
  `vt/ff-01` で P0(worktree 跨ぎの作業ブランチ黙示破壊)を**実コマンドで再現**でき
  (`00b822f` → `58264ed`、git は "Fast-forward" と成功報告)、修正の要否が議論の余地なく確定した。
  合格基準を README に埋め込んだことで verifier が基準を緩めず V1〜V21 を判定した
- **結論**: 変更なし。両ルールは維持する

### 1. [pj-git-route §2 の是正 — 安全性] `--` セパレーター規則の書き換え

- **内容**: 旧「先頭 `-` 拒否があれば `--` は必須ではない」を、remote 位置の引数については
  **必須**に是正。実証コマンド(`git fetch "--upload-pack=echo pwned" <remote>` が echo を実行)と、
  `git check-ref-format refs/heads/-x` が exit 0 =「git 由来だから安全」が成立しないことを併記。
  最も確実な策として「remote をリクエストボディで受け取らない」を明記
- **証拠**: 2026-07-29【驚き】任意コマンド実行の実測
- **効果確認(観測可能)**: 新規の git ルートで remote 位置に `--` の無いコマンドが書かれない。
  reviewer が同型の指摘を出さずに済むこと

### 2. [pj-git-route §2 の是正] 検証純関数の切り出し先を明示

- **内容**: 「切り出し先は `server/index.ts` の export ではなく別モジュール」を追記。
  index.ts に置くとテストが `vi.mock('node:http')` を強いられる(今回実際にそうなった)
- **証拠**: 2026-07-29【摩擦→要判断】index.test.ts のモック
- **効果確認**: 次に検証純関数を追加するとき、モック無しでテストが書けること。
  既存の `checkBranchRefParam` を別モジュールへ移す判断は別途(積み残しとして記録)

### 3. [pj-git-route §5 に 2 行追記] git コマンド固有の定石

- **内容**: (a) ロケール依存を env で潰す前に plumbing コマンドを探す
  (`git branch --format` は翻訳され `for-each-ref` は非翻訳)。(b) ref 一覧のパースは
  名前の形でなく `refs/heads/`・`refs/remotes/` 前置 + `%(symref)` で構造的に判定する
- **証拠**: 2026-07-29【驚き】gettext は builtin/branch.c 経由のみ /
  `%(refname:short)` が `origin/HEAD` を `origin` に短縮 / detached HEAD 擬似行
- **効果確認**: ref 一覧パースに起因する幽霊行・擬似行の不具合が再発しないこと

### 4. [CLAUDE.md に新規節(マーカー外)] 判別器は真理値表を自分で作ってから採用する

- **内容**: 「判別器・ランキング・分類の提案は、誰のものでも(レビュアー・subagent・過去の自分の
  記録も含めて)全分岐の実データに当てて真理値表を自分で作ってから採用する。根拠として添えられた
  事実主張ごと測る。自分が発注した検証が返っていないうちは承認ゲートを開けない」
- **証拠**: **2 ミッション 5 事例** — レビュアー提案の `%(push)` 検出器が三角ワークフローを巻き込む
  誤りだった(2026-07-29)/ inbox 記録済みの制御バイト走査式が 0x0b・0x0c を素通し(2026-07-29)/
  scout のランキングが無罪の箇所を確信度「高」で提示(2026-07-27)/ builder の「未実測だが原理上
  こうなるはず」が誤り(2026-07-27)/ builder のテストが green でも Conductor の入力で穴(2026-07-27)
- **効果確認(観測可能)**: 「推奨をそのまま採用して誤りだった」型のシグナルが inbox に再出現しないこと。
  Conductor が真理値表を自作した記録が journal / 報告に現れること

### 5. [pj-isolated-verify に罠 10・11 を追記]

- **内容**: (10) タイミング依存の検証では `Input.dispatchMouseEvent` の `mouseMoved` を外す
  (1 発 5 秒の遅延)。(11) 制御バイトの走査は許可リスト方式で書く(既記録の除外式は 0x0b/0x0c を素通し)
- **証拠**: 2026-07-28【摩擦】CDP 遅延の実測 / 2026-07-29【驚き】走査式の穴
- **効果確認**: 連打・競合系の検証で「間隔が空いていただけ」の誤判定が起きないこと。
  制御バイト混入がフックだけで検出される事象が再発しないこと

### 6. [pj-client-ui-state に §6 追記] サードパーティの内部 UI 状態は data 差し替えで消えない

- **内容**: react-arborist の開閉状態は `data` prop から独立した内部ストアにあり、`data` を丸ごと
  差し替えても消えない。再取得系は「データと UI 内部状態の両方を整合させる」ことを設計項目に立てる
- **証拠**: 2026-07-28【驚き】ツリー再読み込みで「開いた見た目のまま中身 0 件」
- **効果確認**: 再取得を伴う UI 変更で、開閉・選択・スクロールの不整合が再発しないこと

### 7. [メモリーへ移送] PowerShell のバイト単位書き換え

- 37KB のファイルが 2 バイトに切り詰められた事故(非終了エラーで後続が実行された)。
  Windows 環境の道具知識としてメモリー `powershell-byte-edit.md` に保存し、inbox からは外す

### 8. [据え置き] pj-isolated-verify 罠 4(選択中 worktree 削除のロック)

- 「2026-07-21 以降再現未確認。次に worktree 周りを触るときに確かめ、直っていれば削除」の項目。
  今回 worktree を作成・操作したが**削除は試していない**ため判定できず据え置き。
  次に worktree 削除に触れるとき必ず確認する

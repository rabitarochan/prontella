...
- [ ] 2026-07-27 【失敗→パターン】scout の所見が誤ったのは「ランキングだから」ではなく、**この codebase に "タブ" と呼ばれる別物が 2 つある**ため(タイルのペイン切替 `TileView = files|git|term` と、FilesTab 内部のファイルタブ `tabs[]`)。scout は事実(TileView の全 kind)は正しく列挙したが、推奨先を誤った概念に接続した。**同名概念が複数ある領域では、Conductor が brief の時点で「どちらを指すか」を名指しで固定する**(今回 architect には「TileView の新 kind ではない」と明記して回避)
- [ ] 2026-07-27 【成功パターン】`npm audit` の警告を「自分が入れた依存のせいか」切り分けるには `npm audit --json` の **`nodes` フィールドで実際のパスを見る**。今回 dompurify の advisory は range `<=3.4.11` で、該当ノードは `node_modules/monaco-editor/node_modules/dompurify@3.2.7`(monaco の入れ子・既存)のみ。トップレベルに入れた 3.4.12 は無罪だった。件数だけ見て「依存を足したら脆弱性が増えた」と誤報告する事故を防げる
- [ ] 2026-07-27 【成功パターン】セキュリティ境界にライブラリを選ぶとき、**advisory の一覧はそのライブラリの「使ってはいけない機能」のリスト**として読める。DOMPurify の既知 advisory はほぼ全部が `IN_PLACE` / `ADD_TAGS` の関数形式 / `USE_PROFILES` / hook による設定汚染に集中していたので、builder への制約として「これらの機能を使わない」を明示できた。バージョンを上げるだけで満足しない
- [ ] 2026-07-27 【驚き】`monaco.editor.colorize()` の `.mtkN` 配色 CSS は**エディターインスタンスが 1 度も生成されないと DOM に注入されない**(standaloneThemeService が生成時に `<style class="monaco-colors">` を作る)。「monaco が bundle されていれば colorize は単体で使える」は誤った直感で、プレビュー専用画面ではモノクロになる。検証項目に「エディタータブを一度も開かずに」を入れないとすり抜ける
- [ ] 2026-07-27 【失敗→パターン】Windows の Git Bash から `curl --get --data-urlencode "path=画像/図1.png"` を投げると**非 ASCII が壊れてサーバーが 404 を返す**。サーバーは無罪で、原因は shell の引数エンコーディング。危うく T0 に虚偽のバグ報告を出すところだった。**非 ASCII を含む URL 検証は、shell に触らせず事前にパーセントエンコードした URL 文字列を直接渡す**(`path=%E7%94%BB%E5%83%8F%2F...`)。切り分け手順は「node で `fs.readdirSync` して実際のバイト列を hex で見る」+「pre-encoded URL で再試行」の 2 手 — pj-isolated-verify 追記候補
- [ ] 2026-07-27 【成功パターン】builder の報告の「積み残し・懸念」欄は**最も情報密度が高い**。T4 が自主的に挙げた「`validateLink` を無効化した(セキュリティ境界の非自明な変更)」を Conductor が実地検証したところ、主張は正しかったが**より深い欠陥**(`classifyHref('')` が空 href をディレクトリーへの相対リンクに誤分類。markdown-it を通さなくても `[x]()` で再現する下位関数のバグ)が見つかった。懸念欄の主張は「正しいか」だけでなく「**その原因はもっと下にないか**」まで辿る
- [ ] 2026-07-27 【失敗→パターン】subagent は「防御層が競合したとき、片方を無効化して一本化する」方向に倒しがち(T4 が markdown-it のスキーム検査を切って `classifyHref` に一本化)。二重防御は**役割が違うから 2 枚ある**ので、競合の解消は「片方を消す」ではなく「**両方が同じ安全側の結論に到達するよう下位の欠陥を直す**」。brief で二重防御を要求するときは、この解消方針まで書いておく
- [ ] 2026-07-27 【驚き】**不可視の C0 制御バイトがソースに混入する事象を、別々の builder が独立に 2 回踏んだ**(T3 は制御文字レンジの正規表現リテラルを書こうとして、T4 は空文字列のつもりの箇所に)。どちらも typecheck も vitest も通過し、node でのバイト走査でしか見つからない。Conductor 自身も inbox にこの件を書こうとして Bash ツールの制御文字ガードに弾かれた(＝ Bash には防壁があるが Edit/Write には無い)。**制御文字を扱うコードを書かせたら、完了報告の時点で Conductor がバイト走査を回す**(`c<9||(c>13&&c<32)||c===127` を数える)。実装は正規表現リテラルでなく文字コード比較のループで書かせるのが予防策 — pj-* 追記候補
- [ ] 2026-07-27 【驚き】markdown-it の `validateLink` が false を返すと、実装(`rules_inline/link.mjs`)は href を空にするだけでなく**パース位置を進めないためリンク構文全体がテキストにフォールバックする**。結果 `[x](javascript:alert(1))` は `<a>` 要素すら生成されない。「危険な href が空文字で残る」という自然な想定は誤りで、実際に危険なのは元から空/ディレクトリーのみの `[x]()` `[x](.)` だった。ライブラリの防御挙動は推測せず実際の出力を並べて確認する
- [ ] 2026-07-27 【摩擦】スクラッチ配下に置いた使い捨て vitest 設定からプロジェクトのモジュールを import して検証すると、**third-party 依存の解決経路が変わって挙動が化ける**ことがある(T5 が遭遇: プロジェクト外テストを起点にすると `render.ts` 内の markdown-it の挙動が本番と一致しなかった)。切り分けは「同じスクラッチ設定で本物のテストファイルを走らせて green か」を見る。**スクラッチ検証で得た third-party がらみの否定的所見は、本番の `npm test` で再現するまで所見として採用しない** — pj-isolated-verify 追記候補
- [ ] 2026-07-27 【失敗→パターン】**「正規化・デコードの順序」はセキュリティ判定の定番の穴**。`classifyHref` は生の文字列でスキーム判定し、パーセントデコードは後段の `resolveRelative` 内で走っていたため、`java%09script:` `%6Aavascript:` が全て `blocked` をすり抜けた(`%6A` はデコードすると `javascript:` そのもの)。生の制御文字(`java\tscript:`)は潰せていたのに、そのエンコード形が抜けていた。**「危険な入力の正規化形」ではなく「正規化してから判定」の順序にする**。判定と正規化が別関数に分かれている設計では特に踏みやすい
- [ ] 2026-07-27 【成功パターン】敵対的フィクスチャを Conductor が**実装前に**作っておくと、builder の自己検証で拾えなかった欠陥が統合段階で確実に出る(今回 B3 の難読化スキームが T8 の実機検証で発火)。さらに builder の報告(1 パターン)を Conductor が自分で横展開したら**5 パターンに広がった**。**報告された欠陥は必ず「同じ原因の別の入り口」を自分で探す**
- [ ] 2026-07-27 【成功パターン】reviewer に**推奨修正そのものの副作用まで書かせる**と価値が跳ね上がる。今回 R-1(エラーが成功時にクリアされない)に対し「素直に `error: ''` を足すと、`source` が前回と同一文字列なので effect が再実行されず本文が空になる第 2 のバグが出る」まで指摘してきた。**修正案は「直るか」だけでなく「直したとき何が壊れるか」を必ず添えさせる**
- [ ] 2026-07-27 【摩擦】このプロジェクトは `vitest.config.ts` の include が `client/src/**/*.test.ts` のみで **`.tsx` が対象外**、かつ `@testing-library/react` も無いため、**React コンポーネントの挙動は構造的にユニットテストで守れない**。今回の reviewer 指摘 R-1〜R-3 は全てコード読解ベースで、回帰テストを書く手段が無い。コンポーネントに寄せた不具合は「実機検証でしか捕まらない・回帰も防げない」ことを前提に、検証項目を厚くするか、テスト基盤の追加を別件で検討する
- [ ] 2026-07-27 【失敗→パターン】builder が「実測での比較はしていませんが、アーキテクチャ上そうなる設計です」と書いた断定は**実際に誤りだった**(json フェンスが 1 色なのを「monaco の JSON トークナイザーの特性」と断定 → verifier が『更新ボタンを押すと 28 span で正しく着色される』と反証。真因はコールドスタート時の遅延登録)。**「未実測だが原理上こうなるはず」は所見ではなく仮説**として扱い、仕様として記録しない。反証は「同じ操作をもう一度やる」だけで済むことが多い
- [ ] 2026-07-27 【成功パターン】検証フィクスチャの中に**合格基準そのものを書き込んでおく**と、verifier が独自判断で基準を緩めるのを防げる。今回 `gfm.md` に「エディタータブを一度も開かずにプレビューだけ開いた状態でも着色されること(モノクロなら不合格)」と書いておいたため、ブリーフ本文が名指ししていなかった json の失敗も verifier が自力で不合格判定できた
- [ ] 2026-07-27 【失敗→パターン】**同一関数(`classifyHref`)で「正規化と判定の順序」欠陥が 3 回連続で出た**: (1) パーセントデコードがスキーム判定より後 (2) `\` をパス区切りとして正規化していない (3) バックスラッシュ正規化がプロトコル相対 `//` 判定より後。**1 件直すたびに同じ型が別の入り口で再発する**ので、この型を見つけたら「その 1 件を直す」ではなく**「正規化を全部先に済ませてから全判定を走らせる」構造に作り直させる**のが正解。個別修正を繰り返すと修正サイクルを浪費する(今回 3 サイクル消費)
- [ ] 2026-07-27 【成功パターン】builder の修正報告を Conductor が**独立に自作プローブで再検証**したところ、報告では全件通過とされた修正に 1 件の穴が残っていた(エンコード UNC `%5C%5Chost%5C...`)。**セキュリティ境界の修正は、builder のテストが green でも Conductor が自分の入力セットで叩き直す**。builder のテストは builder が想定した攻撃面しかカバーしない

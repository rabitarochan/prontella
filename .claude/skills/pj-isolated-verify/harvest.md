...
## 013: ホーム直下のツールキャッシュを読む子プロセスは隔離 home で検証しない (原理の例外を拡張)

- date: 2026-09-12
- context: C# 言語サーバー (Roslyn) を隔離 home (`USERPROFILE` 差替、port 4711) で実機検証したところ、
  ソリューションは読めてステータスも ready になるのに補完とホバーが null のまま。ブリッジのコードを
  疑って 2 回プローブを書き直したあと、実 home + 別ポートで起動したら同じ操作で候補が返った。原因は
  Roslyn (MSBuild/NuGet) が `~/.nuget/packages` を隔離 home 側で探して参照を解決できないこと。
  既存の例外条項は「env/PATH の再構成が検証対象のとき」しか挙げておらず、「子プロセスがホーム下の
  キャッシュに依存する」形は想定していなかった。
- change: 原理の例外に「子プロセスがユーザープロフィール下のキャッシュ/設定を読む場合も同じ」を
  追記し、`.nuget` / `.npm` / `.cargo` / `.m2` を例示。Why = 隔離が**検証対象の入力そのもの**
  (パッケージ参照) を変えてしまい、失敗が製品の不具合と区別できない。How = そういう機能は
  最初から実 home + ポート分離 (書き込み経路の事前列挙と事後確認は既存条項どおり) で検証する。
- supersedes: — (既存の例外を広げるだけ。隔離を既定とする方針は維持)
- result: 実 home + 4711 で C# の補完・ホバー・定義ジャンプ・2 ソリューション並行を確認。
  実 config のタイムスタンプ不変、対象 repo の git status 不変を事後確認した。

## 014: claude-in-chrome で Monaco を判定するときの偽陰性 (罠 28)

- date: 2026-09-13
- context: LSP フェーズ 2 の実機検証 (診断・引数表示・参照・root 外ビューアー・外部変更の取り込み)
  を claude-in-chrome で行い、正常な実装を不合格と誤判定しかけた箇所が 4 つあった: view-line の
  DOM 順で末尾行を読んで「入力が反映されていない」と誤認、`type` の改行が落ちて「診断が出ない」と
  誤認、MCP タブの visibilityState が hidden で外部変更の取り込みが走らず「同期しない」と誤認、
  変更直後 1 回の波線カウント 0 を「マーカーが付かない」と誤認。いずれもトレース (サーバー側の
  要求/応答) と hover 文言で裏を取って製品側は正常と確定した。ほかに大きいファイルを載せた直後の
  screenshot タイムアウト、ref クリックで onSelect が 2 回走る現象も踏んだ。
- change: 罠 28 として 4 つの偽陰性と回避 (style.top ソート / Enter キー / visibilityState の強制 /
  待って再読 + hover で裏取り) を追加。Why = ブラウザー自動化の観測は「その瞬間の DOM」であり、
  製品の判定経路 (visibility ガード、仮想化、非同期描画) を通っているとは限らない。How = 判定は
  DOM の 1 回読みではなく、製品側のトレースか安定した文言で確定する。
- supersedes: —
- result: 4 件とも製品側は正常だった。トレース確認を先にしていれば各 2〜3 往復を省けた。
